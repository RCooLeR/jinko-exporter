...
export const ENTITY_DEFINITIONS = {
  ...
  grid_load_total_power: { names: ["Grid Load Total Power"] },
  grid_load_total_current: { names: ["Grid Load Total Current"] },
  grid_load_l1_voltage: { names: ["Grid Load L1 Voltage"] },
  grid_load_l2_voltage: { names: ["Grid Load L2 Voltage"] },
  grid_load_l3_voltage: { names: ["Grid Load L3 Voltage"] },
  grid_load_l1_current: { names: ["Grid Load L1 Current"] },
  grid_load_l2_current: { names: ["Grid Load L2 Current"] },
  grid_load_l3_current: { names: ["Grid Load L3 Current"] },
  grid_load_l1_power: { names: ["Grid Load L1 Power"] },
  grid_load_l2_power: { names: ["Grid Load L2 Power"] },
  grid_load_l3_power: { names: ["Grid Load L3 Power"] },
...
const resolveEntity = (hass: HomeAssistant, key: EntityKey, override?: string): string | null => {
  if (override && hass.states[override]) return override;
  ...
  for (const [entityId, stateObj] of Object.entries(hass.states)) {
    ...
    for (const [index, name] of descriptor.names.entries()) {
      ...
      if (friendly === normalizedName) score = Math.max(score, 300 - index);
      else if (friendly.includes(normalizedName)) score = Math.max(score, 180 - index);

      if (normalizedEntityId.includes(normalizedName)) score = Math.max(score, 120 - index);
    }
...
export const valueFor = (hass: HomeAssistant | undefined, resolved: ResolvedEntityMap, key: EntityKey): number | null => {
  ...
  return entityId ? toNumber(hass?.states[entityId]) : null;
};
